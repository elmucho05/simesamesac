...
class ErrorCodes:
    ...
    def get_error(e):
        return (
            ErrorCodes._descriptions[e] if e in ErrorCodes._descriptions.keys() else ""
        )
...
def ret_format(
    err_code, exp_sc, res_sc, exp_bd, res_bd, req_url, req_met, req_bd
) -> dict:
    return {
        "error": err_code,
        "expected_sc": exp_sc,
...
def assert_equal(r, exp_body, exp_sc, req_body=None):
    try:
        ...
        print(r)
        return ret_format(
            ErrorCodes.INT_ERROR, None, None, None, None, None, None, None
        )
...
class TestEndpoints:
    ...
    def execute_test(self, t):
        t["exp_body"] = (
            None if "exp_body" not in t.keys() else json_parse(t["exp_body"])
        )
...
if __name__ == "__main__":
    ...
    test = TestEndpoints("http://127.0.0.1:8080")
